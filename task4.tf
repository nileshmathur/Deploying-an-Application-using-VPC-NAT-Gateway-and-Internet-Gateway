...
############################################################################################################################################
resource "aws_route_table" "nilesh_nat" {
	vpc_id=aws_vpc.nilesh_vpc.id
	route{
		cidr_block="0.0.0.0/0"
		gateway_id 		= aws_nat_gateway.nilesh_ng.id                 #Creating Routing Table and Associating it with respective subnet

	}
}
resource "aws_route_table_association" "pri_subnet_rt" {
	depends_on=[aws_route_table.nilesh_nat]
	subnet_id=aws_subnet.pri_subnet.id
	route_table_id=aws_route_table.nilesh_nat.id
